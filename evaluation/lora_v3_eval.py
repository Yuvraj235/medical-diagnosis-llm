# -*- coding: utf-8 -*-
"""
evaluation/lora_v3_eval.py
Evaluate the LoRA v3 fine-tuned BioGPT-Large on the PubMedQA test set.

Uses the same correct Microsoft format as training.
Run AFTER models/lora_v3_finetune.py has completed.

Run:
    python evaluation/lora_v3_eval.py
"""
...
LORA_V3_PATH = os.path.join(MODEL_DIR, "lora_v3_best")
...
def run(n_samples=150):
    ...
    logger.info(f"Loading LoRA v3 adapter from {LORA_V3_PATH} ...")
    model = PeftModel.from_pretrained(base_model, LORA_V3_PATH)
    ...
    print("\n" + "=" * 64)
    print("  LoRA v3 Fine-tuned BioGPT-Large (bfloat16 + grad-ckpt)")
    print("=" * 64)
    ...
    print(f"  LoRA v3 delta: {(acc - 0.6533)*100:+.1f}pp")

    result = {
        "method":      "lora_v3_bfloat16_gradckpt",
    ...
    out = f"/Users/yuvrajpratapsingh/medical_rag/results/lora_v3_eval_{int(time.time())}.json"
