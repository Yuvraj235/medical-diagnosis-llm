# -*- coding: utf-8 -*-
"""
models/lora_v3_finetune.py
LoRA v3 — Memory-optimised training on Apple Silicon CPU.

Key improvements over v2:
  1. bfloat16 instead of float32  →  model RAM  ≈ 3 GB  (was 6 GB)
  2. Gradient checkpointing        →  activation RAM halved, no more swap-hell
  3. model.enable_input_require_grads() before PEFT (required for grad-ckpt)
  4. Saves to lora_v3_best (preserves the lora_v2_best 63.3% checkpoint)

Everything else identical to v2 (gold-standard format, class weights, safety
check, 700 samples, r=8, α=16, 4 targets, 5 epochs, early stopping).

Expected runtime:  3–5 hours  (vs  24 h+ with float32 swap)
Expected accuracy: 70%+

Run:
    python models/lora_v3_finetune.py
"""
...
LORA_OUT   = os.path.join(MODEL_DIR, "lora_v3_best")
MODEL_NAME = "microsoft/BioGPT-Large-PubMedQA"

# ── Hypers ───────────────────────────────────────────────────────────────────
MAX_LEN       = 512   # Most PubMedQA sequences < 512 tok; label safety check skips longer ones
BATCH_SIZE    = 1
...
def run_finetune():
    ...
    device = "cpu"
    dtype  = torch.bfloat16   # ← KEY: halves model RAM vs float32
    ...
    logger.info(f"LoRA v3 targets ({len(targets)} modules): {targets}")

    # Required BEFORE get_peft_model when using gradient checkpointing
    ...
    # Enable gradient checkpointing AFTER wrapping with PEFT
    model.gradient_checkpointing_enable()
    logger.info("Gradient checkpointing enabled  (saves activation RAM)")
    ...
    for epoch in range(N_EPOCHS):
        ...
        for rec in train_data:
            ...
            # Safety check: skip any sample where label was truncated away
            expected_label_id = label_tokens[label]
            if input_ids[-1].item() != expected_label_id:
                global_step += 1
                continue

            # Loss on last token only (the label)
    ...
    with open(os.path.join(MODEL_DIR, "lora_v3_history.json"), "w") as f:
        json.dump(history, f, indent=2)
    with open(os.path.join(LORA_OUT, "base_model.txt"), "w") as f:
        f.write(MODEL_NAME)

    total_min = (time.time() - t_start) / 60
    logger.info(f"\n LoRA v3 complete!  ({total_min:.1f} min)")
